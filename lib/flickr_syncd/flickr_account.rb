  ...
  class FlickrAccount
    ...
    def upload_to_set(media_files, set_id = nil)
      ...
      if media_files.any? && (set_id.nil? || set_id.empty?)
        ...
        FlickrSyncd::Photo.create(:id => photo_id, :title => file_path, :set_id => photoset.id, :user_id => FlickrSyncd::Authenticator.user_id)
        FlickrSyncd::Set.create(:flickr_id => set_id, :name => URI.encode(File.dirname(file_path)))
      end
